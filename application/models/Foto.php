...
class Foto extends CI_Model
{
    ...
    public function getOverviewData()
    {
        $overview = array();
//        todo rename columns
        $dbAlbum = $this->db->query('SELECT album.id as id, name, text, place, album.password as password, hash, login FROM `album` JOIN users ON (album.id_user = users.id) WHERE hidden = 0');
